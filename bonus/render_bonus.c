...
{
	while (t->x <= 20.0 + t->xcpy && (t->y >= 0.0 && t->y <= 240.0))
	{
...
{
	t->ycpy = 120.0 - (((t->player_y) / t->g->block_size) * 20.0);
	while (t->real_map[++t->j])
	{
		t->i = -1;
		t->xcpy = 120.0 - (((t->player_x) / t->g->block_size) * 20.0);
		while (t->real_map[t->j][++t->i])
		{
			t->y = t->ycpy;
			while (t->y <= 20.0 + t->ycpy)
			{
				t->x = t->xcpy;
				ft_minimap_wall_x(t);
				t->y += 1.0;
...
void	ft_minimap_player(t_map *t)
{
	...
	t->y = 0.0;
	t->x = 0.0;
	while ((t->x <= 10.0 && t->x >= -10.0) && (t->y <= 10.0 && t->y >= -10.0))
	{
